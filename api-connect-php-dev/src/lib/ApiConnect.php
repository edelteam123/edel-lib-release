...
use com\apiconnect\apiconnect\api\EquityHoldingsApi;
use com\apiconnect\apiconnect\api\EquityLimitsApi;
use com\apiconnect\apiconnect\api\EquityOrdersApi;
use com\apiconnect\apiconnect\api\EquityPositionBookApi;
use com\apiconnect\apiconnect\api\EquityTradeApi;
use com\apiconnect\apiconnect\api\EquityTradeBookApi;
use com\apiconnect\apiconnect\api\CommodityLimitsApi;
use com\apiconnect\apiconnect\api\CommodityOrderBookApi;
use com\apiconnect\apiconnect\api\CommodityOrderDetailsApi;
use com\apiconnect\apiconnect\api\CommodityOrderHistoryApi;
use com\apiconnect\apiconnect\api\CommodityPositionBookApi;
use com\apiconnect\apiconnect\api\CommodityReportsApi;
use com\apiconnect\apiconnect\api\CommodityTradeApi;
use com\apiconnect\apiconnect\api\CommodityTradeBookApi;
use com\apiconnect\apiconnect\api\MFTradeApi;
use com\apiconnect\apiconnect\api\MFHoldingsApi;
use com\apiconnect\apiconnect\api\MFOrderBookApi;

use Logger;
use com\apiconnect\LogConfig;
use GuzzleHttp\Client;
use com\apiconnect\ApiException;
use com\apiconnect\Configs\ApiConfig;
use com\apiconnect\Configuration;
use com\apiconnect\Enums\ActionEnum;
use com\apiconnect\Enums\DurationEnum;
use com\apiconnect\Enums\ExchangeEnum;
use com\apiconnect\Enums\OrderTypeEnum;
use com\apiconnect\Enums\ProductCodeEnum;
use com\apiconnect\Exceptions\ValidationException;
use com\apiconnect\HeaderSelector;
use com\apiconnect\apiconnect\api\ChartsContentApi;
use com\apiconnect\Resources\ChartsResponseFormatter;
use com\apiconnect\Resources\LiveNewsResponse;
use com\apiconnect\Service\LiveNewsService;
use com\apiconnect\Resources\ResearchCallsResource;
use com\apiconnect\Service\ResearchCallsService;
use com\apiconnect\Resources\WatchListResource;
use com\apiconnect\Service\WatchListService;
use com\apiconnect\Validators\EODRequestValidator;
use com\apiconnect\Validators\IntradayRequestValidator;
use com\apiconnect\Validators\LiveNewsRequestValidator;
use com\apiconnect\Validators\LatestCorpActReqValidator;
use com\apiconnect\Validators\ResearchCallsReqValidator;
use com\apiconnect\Validators\WatchListRequestValidator;
use stdClass;
...
class ApiConnect
{
    ...
    {
        try {
            return (new LiveNewsService($this->GetLoginData(), $this->filename, $this->config, $this->apiConfig))->getNewsCategories();
        } catch (ApiException $e) {
            $this->log->error("News Categories :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
     ...
     * @param  mixed $stockSpecific
     * @param  mixed $pgNum
     * @return void
     */
    public function getLiveNews($category, $holdings, $searchText = '', $pgNum = 0)
    {
        try {
            $data = ['cat' => $category, 'holdings' => $holdings, 'searchTxt' => $searchText, 'pgNum' => $pgNum];
            (new LiveNewsRequestValidator($data))->validateData();
            $liveNewsService = new LiveNewsService($this->GetLoginData(), $this->filename, $this->config, $this->apiConfig);
            $result = "";
            if ($holdings) {
                list($result, $body, $userSpecificCat) = $liveNewsService->getEqHoldings($searchText, $pgNum, $category);
            } else {
                list($result, $body) = $liveNewsService->getNewsGeneralData($category, $searchText, $pgNum);
            }

            return json_encode(new LiveNewsResponse($result, $body, $holdings, false, $userSpecificCat ?? []));
        } catch (ApiException $e) {
            $this->log->error("Live News Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("Live News Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
     ...
     * @return void
     */
    public function getNewsForResultsAndStocks($holdings, $searchText = '', $pgNum = 0)
    {
        try {
            $data = ['holdings' => $holdings, 'searchTxt' => $searchText, 'pgNum' => $pgNum];
            (new LiveNewsRequestValidator($data,true))->validateData();
            $liveNewsService = new LiveNewsService($this->GetLoginData(), $this->filename, $this->config, $this->apiConfig);
            $result = "";
            if ($holdings) {
                list($result, $body) = $liveNewsService->getEqHoldings($searchText, $pgNum);
            } else {
                list($result, $body) = $liveNewsService->getNewsForResultsAndStocks($searchText, $pgNum);
            }
            
            return json_encode(new LiveNewsResponse($result, $body, $holdings, true, []));
        } catch (ApiException $e) {
            $this->log->error("Live News Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
    ...
    {
        try {
            (new LatestCorpActReqValidator(['symbol' => $symbol]))->validateData();
            return (new LiveNewsService($this->GetLoginData(), $this->filename, $this->config, $this->apiConfig))->getLatestCorporateActions($symbol);
        } catch (ApiException $e) {
            $this->log->error("LatestCorporateActions Error  :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("LatestCorporateActions Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }

    /**
     * getActiveResearchCalls
     *
     * @param  mixed $segment
     * @param  mixed $term
     ...
     * @return void
     */
    public function getActiveResearchCalls($segment, $term, $marketCap = null)
    {
        try {
            $this->setRequestHeaders();
            (new ResearchCallsReqValidator(['segment' => $segment, 'term' => $term, 'marketCap' => $marketCap]))->validateData();
            if (!empty($marketCap) && $segment != 'EQ')
                throw new ValidationException('Market Cap is only applicable for EQ segment type. Please try again.');
            $response = (new ResearchCallsService($this->config, $this->apiConfig))->getActiveResearchCalls($segment, $term, $marketCap);
            $this->log->debug("Response ActiveResearchCalls : \n" . json_encode($response));
            return json_encode(new ResearchCallsResource($response));
        } catch (ApiException $e) {
            $this->log->error("ResearchCalls Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("ResearchCalls Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }

    /**
     * getClosedResearchCalls
     *
     * @param  mixed $segment
     * @param  mixed $term
     * @param  mixed $action
     ...
     * @return void
     */
    public function getClosedResearchCalls($segment, $term, $action, $fromDate = "", $toDate = "", $recommendationType = "", $marketCap = null)
    {
        try {
            $this->setRequestHeaders();
            (new ResearchCallsReqValidator(['segment' => $segment, 'term' => $term, 'marketCap' => $marketCap, 'action' => $action, 'fromDate' => $fromDate, 'toDate' => $toDate]))->validateData();
            if (!empty($marketCap) && $segment != 'EQ')
                throw new ValidationException('Market Cap is only applicable for EQ segment type. Please try again.');
            $response = (new ResearchCallsService($this->config, $this->apiConfig))->getClosedResearchCalls($segment, $term, $action, $fromDate, $toDate, $recommendationType, $marketCap);
            $this->log->debug("Response ClosedResearchCalls : \n" . json_encode($response));
            return json_encode(new ResearchCallsResource($response));
        } catch (ApiException $e) {
            $this->log->error("Live News Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
    ...
    public function getWatchlistGroups()
    {
        try {
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->getWatchlistGroups();
            $this->log->debug("WatchListGroups Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchListGroups Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("WatchListGroups Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
    ...
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupName]))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->getWatchlistScrips($groupName);
            $this->log->debug("WatchListScrips Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchListScrips Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("WatchListScrips Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
     ...
     * @return void
     */
    public function createWatchlistGroup($groupName, $symbols)
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupName, 'symbols' => $symbols], true))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->createGroup($groupName, $symbols);
            $this->log->debug("WatchList CreateGroup Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchList CreateGroup Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("WatchList CreateGroup Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
     ...
     * @return void
     */
    public function addSymbolsWatchlist($groupName, $symbols)
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupName, 'symbols' => $symbols], true))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->addSymbols($groupName, $symbols);
            $this->log->debug("WatchList AddSymbol Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchList AddSymbol Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("WatchList AddSymbol Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
     ...
     * @return void
     */
    public function deleteSymbolsWatchlist($groupName, $symbols)
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupName, 'symbols' => $symbols], true))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->deleteSymbols($groupName, $symbols);
            $this->log->debug("WatchList DeleteSymbol Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchList DeleteSymbol Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
        } catch (ValidationException $e) {
            $this->log->error("WatchList DeleteSymbol Validation Error :  {$e->getMessage()}");
            throw $e;
        }
    }
     ...
     * @return void
     */
    public function deleteWatchlistGroups(array $groupNames)
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupNames]))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->deleteGroups($groupNames);
            $this->log->debug("WatchList Delete Groups Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchList Delete Groups Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
     ...
     * @return void
     */
    public function renameWatchlistGroup($groupName, $newGroupName)
    {
        try {
            (new WatchListRequestValidator(['groupName' => $groupName, 'newGroupName' => $newGroupName], false, true))->validateData();
            $this->setRequestHeaders();
            $response = (new WatchListService($this->config, $this->apiConfig, $this->constants))->renameGroup($groupName, $newGroupName);
            $this->log->debug("WatchList Rename Group Response : \n" . json_encode($response));
            return json_encode(new WatchListResource($response));
        } catch (ApiException $e) {
            $this->log->error("WatchList Rename Group  Error :  {$e->getMessage()}");
            if ($e->getCode() == 401) {
                $this->removeDataFile();
            }
            throw $e;
